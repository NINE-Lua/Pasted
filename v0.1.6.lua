...
local script = G2L["2f"];
	...
	local function genName()
		return string.char(math.random(97, 122)) .. HttpService:GenerateGUID(false):gsub("-", ""):sub(1, math.random(4, 6))
	end
	
	local function xorEncode(src, key)
		local out = {}
		for i = 1, #src do
			out[#out + 1] = bit32.bxor(string.byte(src, i), key)
		end
		return out
	end
	...
	end]], fn, math.random(1, 500), math.random(2, 6))
			end,
	...
	local function %s(x)
		return (x * %d + %d) %% %d
	end]], fn, math.random(2, 7), math.random(3, 15), math.random(10, 99))
			end,
			...
			function()
				...
				for _ = 1, math.random(4, 8) do
					arr[#arr + 1] = tostring(math.random(0, 255))
				end
				return string.format("local %s = {%s}", tname, table.concat(arr, ", "))
			end,
	
			function()
				local faken = genName()
				return string.format([[loadstring("%s")]], faken)
			end
		}
		local lines = {}
		for _ = 1, math.random(4, 7) do
			table.insert(lines, t[math.random(1, #t)]())
		end
		return table.concat(lines, "\n")
	end
	
	local function createLoaderLayer(encodedTable, key, depth)
		local bName = genName()
		local kName = genName()
		local sName = genName()
		local oName = genName()
		...
		local lines = {
			...
			junkTop,
			"local " .. bName .. " = {" .. table.concat(encodedTable, ", ") .. "}",
			"local " .. kName .. " = " .. key,
			junkMid,
			"local " .. sName .. " = \"\"",
			"for i = 1, #" .. bName .. " do",
			"    " .. sName .. " = " .. sName .. " .. string.char(bit32.bxor(" .. bName .. "[i], " .. kName .. "))",
			"end",
			junkBot,
			"loadstring(" .. sName .. ")()"
		}
		...
		if depth > 1 then
			local newKey = math.random(10, 255)
			local encoded = xorEncode(code, newKey)
			return createLoaderLayer(encoded, newKey, depth - 1)
		else
			return code
		end
	end
	...
	if button and codeBox and codeBox:IsA("TextBox") then
		button.MouseButton1Click:Connect(function()
			...
			local key = math.random(10, 255)
			local encoded = xorEncode(src, key)
			local obfuscated = createLoaderLayer(encoded, key, 2)
	
			codeBox.Text = obfuscated:gsub("\n", " ")
		end)
	end
end;
task.spawn(C_2f);
